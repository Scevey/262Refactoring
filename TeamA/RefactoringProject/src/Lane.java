...
public class Lane extends Thread implements PinsetterObserver {	
	...
	private ArrayList<ScoreCalculator> frameStates;
	private HashMap<String, ArrayList<ScoreCalculator>> playerFrameStates;
	...
	public void receivePinsetterEvent(PinsetterEvent pe) {
			pinsDown = pe.pinsDownOnThisThrow();
	...
	private int getScore( Bowler Cur, int frame) {
		...
		if (!playerFrameStates.containsKey(Cur.getNick())) {
			playerFrameStates.put(Cur.getNick(), new ArrayList<>());
		}
		...
		int totalScore = 0;
		curScore = (int[]) scores.get(Cur);
		//System.out.println(scores.get(Cur));
		//gameStates.get(Cur);
		for (int i = 0; i != 10; i++) {
			cumulScores[bowlIndex][i] = 0;
		}

		int current = 2 * (frame - 1) + ball;
		...
		if (frame == 9) {
			...
			scoreCalculator.setFrameState(scoreCalculator.getIsLastFrame());
			scoreCalculator.setString("last");
			scoreCalculator.setScore(curScore[current]);

		}
		if (ball == 0 && pinsDown == 10 || curScore[current] == 10) {
			scoreCalculator.setFrameState(scoreCalculator.getIsStrike());
			scoreCalculator.setString("strike");
			scoreCalculator.setScore(curScore[current]);

		}
		if (current != 0) {
			if (ball == 1 && (curScore[current] + curScore[current - 1] == 10)) {

				scoreCalculator.setFrameState(scoreCalculator.getIsSpare());
				scoreCalculator.setString("spare");
				scoreCalculator.setScore(curScore[current]);

			}
		} else {

			scoreCalculator.setFrameState(scoreCalculator.getIsStandard());
			scoreCalculator.setString("standard");
			scoreCalculator.setScore(curScore[current]);

		}
		for(int i=0; i<frame; i++ ){
				cumulScores[bowlIndex][i] = getFinalScore((playerFrameStates.get(Cur.getNick()).subList(0,i)));


		}
		if(scoreCalculator.getName()==null){
			scoreCalculator.setString("standard");
			scoreCalculator.setScore(curScore[current]);
		}

		if(scoreCalculator.getName()!=null){
			playerFrameStates.get(Cur.getNick()).add(scoreCalculator);
			cumulScores[bowlIndex][frame -1] += getFinalScore((playerFrameStates.get(Cur.getNick())));



			return getFinalScore((playerFrameStates.get(Cur.getNick())));
		}
		else{
		...
		//System.out.println(scoreCalculator.getScore()+"Scoremansnsnsns");


		//if(i%2==1 && curScore[bowlIndex] )

		//System.out.println(Cur + "  " + playerFrameStates.get(Cur).toString());




	public int getFinalScore(List<ScoreCalculator> states){
		System.out.println(states.size());
		int j = 0;
		int total = 0;
		for (ScoreCalculator current : states) {

			if(current.getName()==null){
				return 0;
			}

			if (current.getName()=="last") {
				if (states.contains(j + 2)) {
					if (current.getIsStrike().equals(true)) {
						total += current.getScore() + states.get(j).getScore() + states.get(j + 1).getScore();
					}
					if (current.getIsSpare().equals(true)) {
						total += current.getScore() + states.get(j).getScore();
					} else {
						total += current.getScore();
					}
				}
				if (states.contains(j + 1)) {
					if (current.getIsStrike().equals(true)) {
						total += current.getScore() + states.get(j).getScore();
					}
					if (current.getIsSpare().equals(true)) {
				...
				if (current.getName()=="strike" && states.contains(j + 1) ) {

					total += current.getScore() + states.get(j).getScore() + states.get(j + 1).getScore();
				}
				if (current.getName()==("spare") && states.contains(j)) {
					total += current.getScore() + states.get(j).getScore();
				} else {
					total += current.getScore();
				}
			...
			//System.out.println(states.get(j).getScore() + " Score");
			j++;
		}
